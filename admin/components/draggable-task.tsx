...
import { Users, GripVertical, Clock, Calendar } from "lucide-react"
...
export function DraggableTask({ task, index, columnId, isDragging, onDragStart, onDragEnd }: DraggableTaskProps) {
      ...
      <CardHeader className="pb-3 pr-2">
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            <CardTitle className="text-sm font-medium leading-tight truncate pr-2">{task.streetName}</CardTitle>
            <CardDescription className="text-xs mt-1 truncate">Priority Score: {task.priorityScore}</CardDescription>
          </div>
          {task.isHighPriority && (
            <Badge variant="destructive" className="text-xs shrink-0 px-1.5 py-0.5 h-auto whitespace-nowrap">
              High Priority
            </Badge>
          )}
        </div>
      ...
      <CardContent className="pt-0 space-y-3">
            ...
            <div className="flex -space-x-1 shrink-0">
              {task.team.slice(0, 3).map((member, index) => (
                <Avatar key={index} className="h-5 w-5 border-2 border-background">
                  <AvatarFallback className="text-xs font-medium">{member}</AvatarFallback>
                </Avatar>
              ))}
              {task.team.length > 3 && (
                <div className="h-5 w-5 rounded-full bg-muted border-2 border-background flex items-center justify-center">
                  <span className="text-xs font-medium text-muted-foreground">+{task.team.length - 3}</span>
                </div>
              )}
            </div>
          </div>
          <Badge variant="outline" className="text-xs shrink-0 px-1.5 py-0.5 h-auto">
            {task.status}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2 text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="truncate">Assigned: {new Date(task.assignedDate).toLocaleDateString()}</span>
          </div>
          <div className="flex items-center gap-1.5 min-w-0">
